...
async def _batch(event):
    ...
    if f'{event.sender_id}' in batch:
        return await event.reply("↢ ولك ما اقدر احتفظ بشئ الان حتي ينتهي حفظ المحتوي الأول")
    async with gagan.conversation(event.chat_id) as conv: 
        if not s:
            ...
            try:
                ...
                #print(e)
                logger.info(e)
                return await conv.send_message("↢ ولك القناه ما فيها هذا العدد من المحتويات؟")
            ...
            try:
                ...
                if value > 1000000:
                    return await conv.send_message("↢ هل تظنني خارق كيف احمل هذا العدد : 1000000 😂")
            except ValueError:
            ...
            cd = await conv.send_message("**↢ جـاري الحفط...**\n\nتم تحـميل: ", 
                                    buttons=[[Button.url("Join CH", url="http://t.me/WX_PM")]])
    ...
    for i in range(len(ids)):
        ...
        try: 
            ...
            await countdown.edit(count_down, 
                                 buttons=[[Button.url("Join Channel", url="https://t.me/wx_pm")]])
            ...
            if int(fw.value) > 300:
                ...
                try:
                    ...
                    if countdown.text != count_down:
                        await countdown.edit(count_down, buttons=[[Button.url("انضم فضلاً", url="http://t.me/WX_PM")]])
        except Exception as e:
            #print(e)
            logger.info(e)
            await client.send_message(sender, f"حدث خطأ أثناء الاستنساخ, سوف تستمر الدفعة\n\n**الخـطا:** {str(e)}")
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/WX_PM")]])
        n = i + 1
        if n == len(ids):
            return -2
...
TEXTING = """
```
لاستخدام الامر مـثال : /batch ثم ضع رابط المحتوي الذي تريده
